...
export function buildBodyTrackerPresentationCandidates({ bodySignals = [], trackerSubmissions = [], playbook = [], now = new Date() } = {}) {
  ...
  for (const body of bodySignals.slice(0, 5)) {
    ...
    const records = [body, ...related.slice(0, 3).map((activity) => trackerSubmissions.find((row) => row.id === activity.id))].filter(Boolean);
    const topic = clean(body.signal || body.symptoms?.[0] || "Body signal");
    const latest = related[0];
    const supporting = latest.fields.filter((field) => field.role === "supporting_metadata").map((field) => `${field.label.toLowerCase()} ${field.value}`).slice(0, 2);
    candidates.push({
      kind: "body_tracker_observation",
      key: `${body.id}:${latest.id}`,
      title: `${topic} has been recorded again`,
      message: `You first recorded ${topic}${body.context ? ` in the context “${body.context}”` : " as a Body signal"}, and ${latest.title} later recorded it${supporting.length ? ` with ${supporting.join(" and ")}` : " again"}. Evidence is beginning to accumulate, but there is not yet enough to identify a reliable pattern or cause.`,
      records,
      significance: 65,
      ...
      now,
      metadata: { bodyRecordId: body.id, trackerActivity: latest, evidenceLevel: records.length >= 3 ? "repeated_observation" : "accumulating_observations" },
    });
  }
  return candidates;
}
