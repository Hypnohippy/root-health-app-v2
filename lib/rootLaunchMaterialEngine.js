...
function getCurrentPicture(snapshot) {
  ...
  return `The current Workforce Wellbeing Index is ${index}. ${concernLine} ${themeLine}`;
}
...
function getPosterFocusPoints(
  ...
  if (concern) {
    points.push(
      `Respond earlier to ${concern.toLowerCase()}.`
    );
  }

  if (focus) {
    points.push(
      `Build practical support around ${focus.toLowerCase()}.`
    );
  }
