...
function getParticipationLine(snapshot) {
  ...
  const workforce = snapshot?.workforceParticipation?.workforceContext;
  if (workforce?.hasRecordedRoster) {
    return `${workforce.activeWorkforceCount} recorded active workforce; ${workforce.invitationSentCount} invitations sent; ${workforce.joinedCount} linked Root memberships joined. ${baselines} baselines and ${followUps} matched follow-ups available.`;
  }
  return `${invited} Root membership record${
    invited === 1 ? "" : "s"
  }, ${joined} activated, ${baselines} baseline${
    baselines === 1 ? "" : "s"
