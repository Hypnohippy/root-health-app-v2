...
import { loadOrganisationActionContext } from "./organisationActions.js";
...
export async function loadAuthorisedHRCoachEvidence({
  ...
  const [
    ...
    supabase.from("wellbeing_assessments").select("id, organisation_id, profile_key, assessment_type, created_at, stress_score, burnout_score, sleep_score, recovery_score, mood_score, focus_score").eq("organisation_id", organisationId).order("created_at", { ascending: true }),
    supabase.from("mind_entries").select("id, organisation_id, profile_key, created_at").eq("organisation_id", organisationId).limit(200),
    supabase.from("journal_entries").select("id, organisation_id, profile_key, created_at").eq("organisation_id", organisationId).limit(200),
    supabase.from("voice_sessions").select("id, organisation_id, profile_key, created_at").eq("organisation_id", organisationId).limit(200),
  ...
  return {
    organisation: { ...organisation, workforceContext: organisationContext.workforce,
      actionContext: await loadOrganisationActionContext(supabase, organisationId) },
    organisationContext,
