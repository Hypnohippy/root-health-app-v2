...
export function buildPersonalInvestigationJournalRow({ profileKey, event, recordedAt = new Date().toISOString() } = {}) {
  const allowedEventTypes = new Set(["started", "abandoned", "resolved", "body_evidence_acknowledged"]);
  ...
  const payload = {
    ...
    acknowledgedBodyEvidenceIds: safeArray(event.acknowledgedBodyEvidenceIds)
...
function buildRelevantEvidence(active, evidence) {
  ...
  const bodyRecords = Array.isArray(evidence?.source?.bodySignals?.currentRecords)
    ? evidence.source.bodySignals.currentRecords
    : evidence?.source?.bodySignals?.records;
  return [
    ...assessments,
    ...selectRecords(evidence?.source?.journalEntries?.records, "journal"),
    ...selectRecords(bodyRecords, "body"),
    ...selectRecords(evidence?.source?.mindEntries?.records, "mind"),
  ].sort((a, b) => new Date(b.recordedAt || 0) - new Date(a.recordedAt || 0));
}

function buildFreshBodyEvidence(active, evidence) {
  const startedAt = new Date(active.startedAt || 0).getTime();
  const bodyRecords = Array.isArray(evidence?.source?.bodySignals?.currentRecords)
    ? evidence.source.bodySignals.currentRecords
    : safeArray(evidence?.source?.bodySignals?.records);
  const acknowledgedIds = new Set(safeArray(active.acknowledgedBodyEvidenceIds));
  return bodyRecords
    .filter((record) => timestamp(record) >= startedAt)
    .filter((record) => !acknowledgedIds.has(record?.id))
    .map((record) => ({
      id: record.id,
      type: "body",
      recordedAt: record.created_at || null,
      intensity: Number.isFinite(Number(record.intensity)) ? Number(record.intensity) : null,
      summary: cleanText(
        [
          record.location_detail || record.areas?.[0],
          safeArray(record.symptoms).length ? record.symptoms.join(", ") : record.signal,
          Number.isFinite(Number(record.intensity)) ? `${record.intensity}/10` : "",
        ].filter(Boolean).join(" · "),
        220
      ),
      relationship: "potentially_relevant_unassessed",
      provenance: {
        layer: "source_record_projection",
        table: "body_signals",
        sourceRecordId: record.id || null,
    ...
    .sort((first, second) => {
      const significance = Number(second.intensity || 0) - Number(first.intensity || 0);
      return significance || new Date(second.recordedAt || 0) - new Date(first.recordedAt || 0);
    })
    .slice(0, 3);
}
...
export function deriveActivePersonalInvestigation({ journalEntries = [], evidence = null, assessmentKnowledge = null } = {}) {
  ...
  events.forEach((event) => {
    ...
    } else if (event.eventType === "body_evidence_acknowledged" && states.has(event.investigationId)) {
      const current = states.get(event.investigationId);
      states.set(event.investigationId, {
        ...current,
        acknowledgedBodyEvidenceIds: [...new Set([
          ...safeArray(current.acknowledgedBodyEvidenceIds),
          ...safeArray(event.acknowledgedBodyEvidenceIds),
        ])],
        sourceRecordIds: [...current.sourceRecordIds, event.sourceRecordId].filter(Boolean),
      });
  ...
  const freshBodyEvidence = buildFreshBodyEvidence(active, evidence);
  ...
  return {
    active: {
      ...
      provenance: {
        ...
        sourceRecordIds: [...active.sourceRecordIds, ...relevantEvidence.map((item) => item.id), ...freshBodyEvidence.map((item) => item.id)].filter(Boolean),
...
export function buildActiveInvestigationFocusReply(active) {
  ...
  const relevantIds = new Set(safeArray(active.relevantEvidence).map((item) => item.id));
  const freshBody = safeArray(active.freshBodyEvidence).find((item) => !relevantIds.has(item.id));
  ...
  const freshLine = freshBody
    ? ` You have also recorded a new Body signal (${freshBody.summary}). That is potentially relevant evidence to keep alongside this investigation, but Root does not yet know whether it is related.`
    : "";
  return `You asked Root to help you understand ${active.label}, so that remains the clearest focus rather than starting a generic topic.\n\n${active.reconciledSummary} ${evidenceLine}${freshLine}\n\n${active.whatRemainsUnknown}\n\n${active.nextQuestion}`;
}

function bodyEvidenceIdsAcknowledgedByFocusReply(active) {
  const freshIds = new Set(safeArray(active?.freshBodyEvidence).map((item) => item?.id).filter(Boolean));
  const relevant = safeArray(active?.relevantEvidence).filter((item) => item.type !== "check_in").slice(0, 2);
  const relevantIds = new Set(safeArray(active?.relevantEvidence).map((item) => item?.id).filter(Boolean));
  const acknowledged = relevant
    .filter((item) => item.type === "body" && freshIds.has(item.id))
    .map((item) => item.id);
  const separatelyNamed = safeArray(active?.freshBodyEvidence).find((item) => !relevantIds.has(item.id));
  if (separatelyNamed?.id) acknowledged.push(separatelyNamed.id);
  return [...new Set(acknowledged)];
}

export function buildBodyEvidenceAcknowledgementEvent(active) {
  const ids = bodyEvidenceIdsAcknowledgedByFocusReply(active);
  if (!active?.id || !active?.issueKey || !ids.length) return null;
  return {
    eventType: "body_evidence_acknowledged",
    investigationId: active.id,
    issueKey: active.issueKey,
    label: active.label,
    acknowledgedBodyEvidenceIds: ids,
    userStatement: "Root acknowledged new Body evidence while continuing this investigation.",
  };
}
