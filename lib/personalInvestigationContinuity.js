...
const CHANGE_PATTERNS = [
  /\b(?:instead|change|switch)\b.*\b(?:focus|explore|work on|look at)\b/i,
  /\b(?:focus|explore|work on|look at)\b.*\b(?:instead|now)\b/i,
];
...
function issueForText(text = "") {
  ...
  return ISSUE_DEFINITIONS.find((issue) => issue.aliases.some((alias) => lower.includes(alias))) || null;
}
...
export function detectPersonalInvestigationIntent(message, activeInvestigation = null) {
  ...
  const starts = issue && (
    ...
    (activeInvestigation && issue.key !== activeInvestigation.issueKey && CHANGE_PATTERNS.some((pattern) => pattern.test(text)))
  );
