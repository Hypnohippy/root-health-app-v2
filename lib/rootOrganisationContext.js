import { withWorkforceContext } from "./organisationWorkforceContext.js";
import { latestOrganisationReviews } from "./organisationLearningHistory.js";
...
function buildUnitSummary({
  ...
  return {
    ...
    recorded_workforce_count: workforce?.hasRecordedRoster
      ? workforce.units.filter(row => includedUnitIds.has(row.unitId)).reduce((sum, row) => sum + row.activeWorkforceCount, 0) : null,
    membership_denominator: employees.length,
    employee_count: workforce?.hasRecordedRoster
      ? workforce.units.filter(row => includedUnitIds.has(row.unitId)).reduce((sum, row) => sum + row.activeWorkforceCount, 0)
      : employees.length,
...
export async function buildOrganisationContext({
  ...
  const [
    ...
    latestOrganisationReviews(supabase, organisationId),
  ]);
  ...
  const unitSummaries =
    units.map((unit) =>
      buildUnitSummary({
        ...
        unitsById,
        childrenByParent,
        workforce: organisation.workforceContext,
      })
    );
  ...
  return {
    ...
    structure: {
      units,
      tree,
      unitCount: organisation.workforceContext.unitCount,
      unitSummaries,
    },

    people: {
      recordedActiveWorkforceCount: organisation.workforceContext.hasRecordedRoster ? organisation.workforceContext.activeWorkforceCount : null,
      employeeMembershipCount: employees.length,
      participationDenominator: "employee Root memberships",
      members,
      ...
      employeeCount: organisation.workforceContext.hasRecordedRoster
        ? organisation.workforceContext.activeWorkforceCount : employees.length,
