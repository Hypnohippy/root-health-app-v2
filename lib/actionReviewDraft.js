...
export function prepareActionReview(entry) {
  ...
  for (const raw of entry.content.split(/\r?\n/)) {
    ...
    if (!line) continue;
    // An explicitly formatted list heading, not an arbitrary list sentence.
    const actionHeading = raw.match(/^\s*(?:\d+[.)]|[-*•])\s+(?:\*\*|__)((?:Increase|Improve|Review|Agree|Assign|Reduce|Establish|Update|Invite)\b[^\r\n]*?)(?:\*\*|__)(?:\s*[:—–-].*)?\s*$/i);
    if (actionHeading) { sections.title ||= actionHeading[1].replace(/:$/, '').trim(); current = null; continue; }
