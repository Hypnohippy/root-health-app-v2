...
export function applyMindPostScoreCompletion(state, score, completion) {
  return completion?.success === true
    ? recordMindAfterScore(state, score)
    : state;
}

export function canSubmitMindPostScore(state, saving = false) {
  return state?.stage === MIND_OUTCOME_STAGES.POST_SCORE && saving !== true;
}
