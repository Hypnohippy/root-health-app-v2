...
export function detectVoicePlaybookOffer(transcript = "") {
  const text = String(transcript || "").trim();
  if (!PLAYBOOK_OFFER.test(text) || !PLAYBOOK_OFFER_ACTION.test(text)) return null;

  // A genuine offer/promise to create or place something in Playbook is enough.
  // Do not depend on one scripted phrase such as "shall I".
