...
export function isCompleteVoicePlaybookContent(transcript = "") {
  const text = String(transcript).trim();
  if (!/\btitle\s*:/i.test(text) || text.length < 80 || text.endsWith("?")) {
    return false;
  }
...
export function classifyPlaybookConflictChoice(transcript = "") {
  const text = String(transcript || "").trim().toLowerCase();
  if (!text) return null;

  if (
    /\b(new|new one|new entry|separate|another|keep both|keep the old|don't overwrite|do not overwrite|save both)\b/i.test(text)
  ) {
    return "new";
  }

  if (
    /\b(overwrite|replace|update|existing|old one|same one|replace it|update it|overwrite it)\b/i.test(text)
  ) {
    return "overwrite";
  }
  ...
  if (!response.ok || !result.ok) {
    if (response.status === 409 && result.conflict) {
      return {
        ...
        error: result.error || "A matching Playbook entry already exists.",
      };
    }
