...
const CREATE_OFFER = /\b(?:shall (?:i|we)|should (?:i|we)|would you like me to|do you want me to|i can|can i|let me)\b[\s\S]*\b(?:create|prepare|make|build|add|save)\b/i;
...
export function detectVoicePlaybookOffer(transcript = "") {
  const text = String(transcript || "").trim();
  if (!PLAYBOOK_OFFER.test(text) || !CREATE_OFFER.test(text)) return null;
