...
export function buildLongitudinalMemory({
  ...
  const recurringBodyPattern = bodyAreas.find(([, count]) => count >= 2) || null;
  const recurringEmotionalTheme = emotionalThemes.find(([, count]) => count >= 2) || null;
  ...
  if (recurringEmotionalTheme && recurringBodyPattern) {
    headline = `Root has grouped several entries around ${recurringEmotionalTheme[0]} and recorded ${recurringBodyPattern[0]} more than once.`;
    reflection =
      "Both have recurred in their own records. Root does not yet have enough evidence to say that they occur together or that one causes the other.";
  }
...
if (recentAverage !== null && olderAverage !== null) {
  ...
  topBodyPatternCount: bodyAreas[0]?.[1] || 0,
  topEmotionalTheme: emotionalThemes[0]?.[0] || null,
  topEmotionalThemeCount: emotionalThemes[0]?.[1] || 0,
  ...
  provenance: {
    ...
    engine: "rootLongitudinalEngine",
    derivationType: "deterministic_occurrence_counts",
    minimumOccurrencesForRecurrence: 2,
    sourceRecordIds: [...recentBody, ...recentJournal, ...recentMind]
      .map((entry) => entry?.id)
      .filter(Boolean),
    sourceTimestamps: [...recentBody, ...recentJournal, ...recentMind]
