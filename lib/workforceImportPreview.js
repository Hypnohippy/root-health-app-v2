...
export function buildWorkforcePreview({ rows = [], columns = [], mappings = {}, existingMembers = [] } = {}) {
  ...
  canonicalRows.forEach((row) => {
    const division = row.division || "Whole organisation";
    const department = row.department || "Unplaced department";
    const team = row.team || "";
    if (!divisions.has(division)) divisions.set(division, new Map());
    const departments = divisions.get(division);
    if (!departments.has(department)) departments.set(department, new Set());
    if (team) departments.get(department).add(team);
  });
  ...
  return {
    ...
    hierarchy: [...divisions.entries()].map(([name, departments]) => ({
      name,
      type: name === "Whole organisation" ? "organisation" : "division",
      departments: [...departments.entries()].map(([departmentName, teams]) => ({
        name: departmentName,
        type: "department",
        teams: [...teams].map((teamName) => ({ name: teamName, type: "team" })),
      })),
    })),
  };
}
