...
export async function getRootMeasurementHistory({
  ...
  if (
    personalContext?.scope === "personal" &&
    ...
    if (!user || user.id !== personalContext.userId) {
      throw new Error("Personal measurement identity could not be verified.");
    }
    ...
    identity = await getRootMemoryIdentity();
  }

console.log(
  "ROOT MEMORY MEASUREMENT IDENTITY:",
  identity
);
