...
export function validateHierarchyFields(hierarchyFields = [], canonicalRows = [], mappedHierarchyFields = null) {
  if (!Array.isArray(hierarchyFields) || !hierarchyFields.length) return false;
  if (new Set(hierarchyFields).size !== hierarchyFields.length) return false;
  if (mappedHierarchyFields !== null) {
    if (!Array.isArray(mappedHierarchyFields) || !mappedHierarchyFields.length) return false;
    if (new Set(mappedHierarchyFields).size !== mappedHierarchyFields.length) return false;
    if (!mappedHierarchyFields.every((field) => STRUCTURE_FIELDS.has(field))) return false;
    if (!hierarchyFields.every((field) => mappedHierarchyFields.includes(field))) return false;
  }
  return hierarchyFields.every(
    (field) => STRUCTURE_FIELDS.has(field) && canonicalRows.some((row) => String(row?.[field] || "").trim())
  );
}
...
export function buildWorkforceRpcPayload(plan) {
  return {
    hierarchyFields: [...(plan.hierarchyFields || [])],
