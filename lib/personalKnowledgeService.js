...
import { compactTrackerActivity, orderPersonalEvidenceChronologically } from "./personalSemanticEvidence.js";
...
export function buildPersonalRootKnowledgeFromEvidence({
  ...
  const knowledge = buildRootKnowledge({
    ...
    trackerSubmissions: records(evidence.source?.trackerSubmissions),
...
export function buildInsightsKnowledgeProjection({
  ...
  return {
    ...
    evidence: {
      ...
      interventionOutcomes: records(evidence.source?.interventionOutcomes),
      trackerSubmissions: records(evidence.source?.trackerSubmissions),
      latestTrackerActivity: compactTrackerActivity(
        orderPersonalEvidenceChronologically(records(evidence.source?.trackerSubmissions), "desc")[0] || null,
        records(evidence.awareness?.playbook)
      ),
    },
    assessmentKnowledge: knowledge.assessmentKnowledge,
...
export function buildCoachKnowledgeProjection({
  ...
  return {
    ...
    trackers: {
      totalSubmissions: knowledge.trackerKnowledge?.totalSubmissions || 0,
      recent: knowledge.trackerKnowledge?.latest || [],
      interpretation: knowledge.trackerKnowledge?.interpretation || "No tracker observations yet.",
    ...
    promptEvidence: {
      ...
      journalEntries: records(evidence.source?.journalEntries)
        ...
        .slice(0, 5),
      trackerSubmissions: records(evidence.source?.trackerSubmissions).slice(0, 10),
    },
    loadStatus: evidence.loadStatus,
