export function buildRootMemoryService({
  ...
  if (firstName) {
    ...
    memory += "These observations are still forming, but the picture is becoming clearer over time.";
  }
  ...
  if (firstName && mostUsedIntervention) {
    ...
    if (helpedCount > 0) {
      interventionInsight = `${toolName} has been used ${totalUses} time${totalUses === 1 ? "" : "s"} recently, and ${helpedCount} response${helpedCount === 1 ? "" : "s"} suggested it helped your system settle.`;
    } else {
