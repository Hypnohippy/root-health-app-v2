export function buildRootMemoryService({
  name = "",
  bodySignals = [],
  journalEntries = [],
  mindEntries = [],
  interventionOutcomes = [],
}) {
  const firstName = name?.trim()?.split(" ")?.[0] || "";

  const emotionCounts = {};
  const bodyCounts = {};
  const interventionCounts = {};

  mindEntries.forEach((entry) => {
    if (entry.emotion) {
      emotionCounts[entry.emotion] = (emotionCounts[entry.emotion] || 0) + 1;
    }

    if (entry.tool && !/^(root\s+)?measurement\s*[—–-]\s*(before|after)$/i.test(entry.tool.trim())) {
      interventionCounts[entry.tool] =
        (interventionCounts[entry.tool] || 0) + 1;

    }
  });

  bodySignals.forEach((entry) => {
    const signal =
      entry.signal || entry.area || entry.body_area || entry.areas?.[0];

    if (!signal) return;

    bodyCounts[signal] = (bodyCounts[signal] || 0) + 1;
  });

  const mostCommonEmotion =
    Object.entries(emotionCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || "";

  const mostCommonBodySignal =
    Object.entries(bodyCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || "";

  const mostUsedIntervention =
    Object.entries(interventionCounts).sort((a, b) => b[1] - a[1])[0];

  const measuredOutcomes = Array.isArray(interventionOutcomes)
    ? interventionOutcomes.filter((outcome) => {
        if (
          outcome?.before_score === null ||
          outcome?.before_score === undefined ||
          outcome?.before_score === "" ||
          outcome?.after_score === null ||
          outcome?.after_score === undefined ||
          outcome?.after_score === ""
        ) {
          return false;
        }

        const before = Number(outcome?.before_score);
        const after = Number(outcome?.after_score);
        return (
          outcome?.completed === true &&
          Number.isFinite(before) &&
          Number.isFinite(after)
        );
      })
    : [];
  const totalMindUses = mindEntries.filter(
    (entry) => !/^(root\s+)?measurement\s*[—–-]\s*(before|after)$/i.test(String(entry?.tool || "").trim())
  ).length;
  const positiveRecoveryCount = measuredOutcomes.filter(
    (outcome) => Number(outcome.before_score) > Number(outcome.after_score)
  ).length;
  const mostCommonEmotionCount = mostCommonEmotion
    ? emotionCounts[mostCommonEmotion] || 0
    : 0;
  const mostCommonBodySignalCount = mostCommonBodySignal
    ? bodyCounts[mostCommonBodySignal] || 0
    : 0;
  const hasRepeatedEmotion = mostCommonEmotionCount >= 2;
  const hasRepeatedBodySignal = mostCommonBodySignalCount >= 2;
const latestThoughtThemeEntry = Array.isArray(mindEntries)
  ? mindEntries.find((entry) => entry.thought_theme)
  : null;
const latestThoughtTheme = latestThoughtThemeEntry?.thought_theme || "";
const latestThoughtNotice = latestThoughtThemeEntry?.thought_notice || "";
 let recognition = "";
 let recognitionConfidence = "none";

if (firstName && latestThoughtTheme) {
  recognitionConfidence = "early";
  recognition =
    `${firstName}, last time Thought Work grouped the entry under ${latestThoughtTheme}. ` +
    `${latestThoughtNotice ? `Root's interpretation was: ${latestThoughtNotice}` : "That classification may be worth reviewing gently."}`;}
else if (firstName && positiveRecoveryCount >= 3) {
  recognitionConfidence = "developing";
  recognition = `${firstName}, Root noticed something worth recognising. Recovery has not been instant, but there are signs your system is finding its way back more often.`;  } else if (firstName && mostCommonEmotion && mostCommonBodySignal) {
    recognitionConfidence = hasRepeatedEmotion && hasRepeatedBodySignal
      ? "developing"
      : "early";
    recognition = hasRepeatedEmotion && hasRepeatedBodySignal
      ? `${firstName}, ${mostCommonEmotion} and ${mostCommonBodySignal} have each been recorded more than once. Root does not yet know whether they occur together.`
      : `${firstName}, Root has one early emotional observation and one body observation. It is too soon to call that a pattern.`;
  } else if (firstName && mostCommonBodySignal) {
    recognitionConfidence = hasRepeatedBodySignal ? "developing" : "early";
    recognition = hasRepeatedBodySignal
      ? `${firstName}, ${mostCommonBodySignal} has been recorded more than once.`
      : `${firstName}, you recorded ${mostCommonBodySignal} once. Root will not treat one entry as a pattern.`;
  }

  let memory = "";

  if (firstName) {
    if (hasRepeatedEmotion && hasRepeatedBodySignal) {
      memory = `${firstName}, ${mostCommonEmotion} and ${mostCommonBodySignal} have each appeared in more than one record.

Root has not established that they occur together, and it will keep the two observations separate until there is stronger evidence.`;
    } else if (hasRepeatedEmotion) {
      memory = `${firstName}, ${mostCommonEmotion} has been recorded more than once.

This does not define the whole story, but it may be one emotional thread worth watching gently.`;
    } else if (hasRepeatedBodySignal) {
      memory = `${firstName}, ${mostCommonBodySignal} has been recorded more than once.

This may become one of the signals worth listening to rather than pushing past.`;
    } else {
      memory = `${firstName}, Root is still gathering the story.

Every check-in helps Root understand a little more about what life feels like from your side of the screen.`;
    }
  }

  let interventionInsight = "";

  let interventionEffectivenessEvidence = false;

  if (firstName && mostUsedIntervention) {
    const [toolName, totalUses] = mostUsedIntervention;
    const toolOutcomes = measuredOutcomes.filter(
      (outcome) =>
        String(outcome?.intervention_name || "").trim().toLowerCase() ===
        String(toolName).trim().toLowerCase()
    );
    const improved = toolOutcomes.filter(
      (outcome) => Number(outcome.before_score) > Number(outcome.after_score)
    ).length;
    const worsened = toolOutcomes.filter(
      (outcome) => Number(outcome.before_score) < Number(outcome.after_score)
    ).length;

    if (improved === 1 && toolOutcomes.length === 1) {
      interventionEffectivenessEvidence = true;
      interventionInsight = `${toolName} may have helped once in a measured attempt. You have used Mind tools ${totalMindUses} time${totalMindUses === 1 ? "" : "s"}; ${measuredOutcomes.length} use${measuredOutcomes.length === 1 ? " has" : "s have"} before-and-after measurements. Root will keep watching before treating that as a reliable pattern.`;
    } else if (improved >= 2 && worsened === 0) {
      interventionEffectivenessEvidence = true;
      interventionInsight = `${toolName} has appeared helpful across ${improved} measured attempts. You have used Mind tools ${totalMindUses} times; ${measuredOutcomes.length} uses have before-and-after measurements. This is a developing personal pattern, not proof of cause.`;
    } else if (improved > 0 && worsened > 0) {
      interventionEffectivenessEvidence = true;
      interventionInsight = `${toolName} has had mixed measured results so far. Root will not assume it is consistently helpful.`;
    } else {
      interventionInsight = `${toolName} has been used ${totalUses} time${
        totalUses === 1 ? "" : "s"
      } recently. Usage alone does not show whether it helped.`;
    }
  }

  let hypothesis = "";

  if (firstName && hasRepeatedEmotion && hasRepeatedBodySignal) {
    hypothesis = `${firstName}, Root is curious whether ${mostCommonEmotion} and ${mostCommonBodySignal} may sometimes be related.

It has not established timing, association, or cause, so it will keep watching without turning this into a conclusion.`;
  } else if (firstName && hasRepeatedEmotion) {
    hypothesis = `${firstName}, Root is curious whether ${mostCommonEmotion} is becoming one of the emotional threads in your recent wellbeing story.

A few more check-ins may help the picture become clearer.`;
  } else if (firstName && hasRepeatedBodySignal) {
    hypothesis = `${firstName}, Root is curious whether ${mostCommonBodySignal} may be one of the early signals your body uses when pressure starts to build.

Root will keep watching this gently over time.`;
  }

  let dailyReflection = "";

  if (positiveRecoveryCount >= 3) {
    dailyReflection = `${firstName}, Root noticed signs of improvement recently.

Some of your recent check-ins suggest that symptoms which once felt louder may be beginning to soften.

Before looking for the next problem to solve, it may be worth pausing to recognise that change.`;
  } else if (hasRepeatedEmotion && hasRepeatedBodySignal) {
    dailyReflection = `${firstName}, ${mostCommonEmotion} and ${mostCommonBodySignal} have each been recorded more than once.

Root has not established that they form one pattern. For now, it is simply keeping both observations in view.

You do not need to solve it today. Simply noticing it may be enough.`;
  } else {
    dailyReflection = `${firstName}, Root is still gathering the story.

Every check-in helps Root understand a little more about what life feels like from your side of the screen.`;
  }

  const memoryData = {
    firstName,
    recognition,
    recognitionConfidence,
    memory,
    interventionInsight,
    hypothesis,
    dailyReflection,
    mostCommonEmotion,
    mostCommonBodySignal,
    mostCommonEmotionCount,
    mostCommonBodySignalCount,
    positiveRecoveryCount,
    interventionEffectivenessEvidence,
    provenance: {
      layer: "derived_observation_set",
      engine: "rootMemoryService",
      derivationType: "deterministic_counts_and_measurements",
      sourceRecordIds: [...bodySignals, ...journalEntries, ...mindEntries, ...measuredOutcomes]
        .map((entry) => entry?.id)
        .filter(Boolean),
      sourceTimestamps: [...bodySignals, ...journalEntries, ...mindEntries, ...measuredOutcomes]
        .map((entry) => entry?.completed_at || entry?.started_at || entry?.created_at)
        .filter(Boolean),
      evidenceOrigins: ["user_entered", "system_classification", "system_measured"],
    },
  };

  return {
    ...memoryData,
    mostImportantObservation: getMostImportantObservation(memoryData),
  };
}

export function getMostImportantObservation(memory = {}) {
  if (memory.recognition) {
    return {
      type: "recognition",
      title: "Something worth recognising",
      message: memory.recognition,
      confidence: memory.recognitionConfidence || "early",
      provenance: memory.provenance || null,
    };
  }

  if (memory.interventionInsight) {
    return {
      type: "intervention",
      title: memory.interventionEffectivenessEvidence
        ? "Something that may be helping"
        : "A tool you have tried",
      message: memory.interventionInsight,
      confidence: memory.interventionEffectivenessEvidence
        ? "early"
        : "usage_only",
    };
  }

  if (memory.hypothesis) {
    return {
      type: "hypothesis",
      title: "Something Root is wondering",
      message: memory.hypothesis,
      confidence: "early",
    };
  }

  if (memory.memory) {
    return {
      type: "memory",
      title: "Root is still learning",
      message: memory.memory,
      confidence: "forming",
    };
  }

  return null;
}
