...
function parseOrganisationSize(
  ...
  if (organisation?.workforceContext?.hasRecordedRoster) {
    const count = organisation.workforceContext.activeWorkforceCount;
    return { minimum: count, maximum: count, estimated: count, source: "recorded active workforce", raw: count };
  }
...
function buildInterventionReadiness({
  confidence,
  evidenceGaps,
  contradictions,
  longitudinal,
  ...
  const organisationConfidence =
    confidence?.organisation?.label || "None";

  const respondentConfidence =
    confidence?.respondents?.label || "None";

  const repeatParticipants =
    longitudinal?.repeatParticipants || 0;

  const qualitativeLevel =
    qualitative?.label || "None";
  ...
  const availableObservations =
    safeArray(observations).filter(
      (observation) =>
        observation.available &&
        Number.isFinite(observation.average)
    );
  ...
  const hasMixedEvidence =
    safeArray(contradictions).length > 0;

  const hasUsefulQualitativeContext =
    ["Moderate", "High"].includes(
      qualitativeLevel
    );

  const hasUsefulLongitudinalEvidence =
    repeatParticipants >= 5;

  const hasStrongEnoughOrganisationEvidence =
    ["Moderate", "High"].includes(
      organisationConfidence
    );

  const hasStrongEnoughRespondentEvidence =
    ["Moderate", "High"].includes(
  ...
  /*
    LEVEL 1
    Evidence is not yet strong enough to select
    a specific organisational intervention.
  */

  if (
    !hasMeasuredSignal ||
    highPriorityGaps.length > 0 ||
    !hasStrongEnoughOrganisationEvidence
  ) {
    return {
      ready: false,

      level: "Not yet supported",

      permittedAction:
        "Evidence-building action",

      reason:
        "The current evidence identifies wellbeing patterns but does not yet provide enough organisation-wide confidence to select a specific intervention.",

      rootPosition:
        "Act on the evidence gap before buying a solution.",

      allowedRecommendations: [
        "Confirm the eligible workforce or programme denominator.",
        "Increase representative participation where appropriate.",
        "Continue consistent wellbeing measurement.",
        "Gather privacy-safe qualitative context.",
        "Compare sufficiently large cohorts where anonymity can be protected.",
      ],

      prohibitedRecommendations: [
        "Specific wellbeing products",
        "Resilience training",
        "Stress-management training",
        "Mental-health awareness training",
        "Manager wellbeing training",
        "Workshops selected solely from assessment scores",
        "Organisation-wide programmes selected without sufficient supporting evidence",
      ],
    };
  }
  ...
  /*
    LEVEL 2
    Evidence supports a measured pilot,
    but not a broad rollout.
  */

  if (
    hasStrongEnoughOrganisationEvidence &&
    hasStrongEnoughRespondentEvidence &&
    (
      hasUsefulLongitudinalEvidence ||
      hasUsefulQualitativeContext
    )
  ) {
    return {
      ready: true,

      level: "Pilot supported",

      permittedAction:
        "Targeted measurable pilot",

      reason:
        "The evidence is sufficiently developed to test a proportionate intervention, but not to assume that a broad organisational rollout will be effective.",

      rootPosition:
        "Test before scaling.",

      requirements: [
        "Define the evidence-supported problem before selecting the intervention.",
        "Define the intended measurable outcome.",
        "Record the baseline before the pilot begins.",
      ...
      caution:
        hasMixedEvidence
          ? "The evidence remains mixed, so any pilot should test a clearly defined hypothesis rather than assume a single explanation."
          : "A supported pilot is still a test, not proof that the intervention will work.",
    };
  }

  /*
    INTERMEDIATE POSITION
    There is useful evidence, but Root still
    needs greater depth before selecting a pilot.
  */
  ...
  return {
    ...
    reason:
      "The evidence is stronger than an initial baseline but does not yet contain enough longitudinal or qualitative support to justify selecting a specific intervention.",

    rootPosition:
      "Strengthen the explanation before selecting the solution.",

    allowedRecommendations: [
      "Continue longitudinal measurement.",
      "Gather privacy-safe qualitative context.",
      "Test whether the pattern persists across sufficiently large cohorts.",
      "Define the outcome that a future intervention would need to improve.",
    ],

    prohibitedRecommendations: [
      "Specific purchased interventions",
      "Broad organisation-wide wellbeing programmes",
      "Solutions selected solely because one assessment dimension has the highest average",
    ],
  };
}
