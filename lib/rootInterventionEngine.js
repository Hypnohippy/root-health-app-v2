...
export async function startIntervention({
  ...
  try {
    ...
    const row = {
      ...
      completed_at: null,
      intervention_id: cleanText(interventionId, 200),
      intervention_slug: cleanText(interventionSlug, 200),
      intervention_version:
        interventionVersion === null || interventionVersion === undefined
          ? null
          : cleanText(String(interventionVersion), 80),
    };
...
export async function completeIntervention({
  ...
  try {
    ...
    const safeProfileKey = cleanText(profileKey, 200);
    const safeAfterScore = normaliseDifficultyScore(afterScore);
    ...
    const change = calculateInterventionChange(
      existing?.before_score,
      safeAfterScore
    );
    const completion = {
      after_score: safeAfterScore,
      completed: true,
      user_observation: cleanText(userObservation, 1000),
      completed_at: new Date().toISOString(),
    };

    if (change !== null) {
      completion.score_change = change;
      completion.improvement_points = change;
    }
...
/**
 * Attach optional subjective feedback after the measured completion without
 * changing either score or creating another intervention attempt.
 */
export async function recordInterventionObservation({
  interventionId,
  profileKey,
  userObservation = null,
} = {}) {
  try {
    ...
    const safeId = cleanText(interventionId, 200);
    const safeProfileKey = cleanText(profileKey, 200);
    const safeObservation = cleanText(userObservation, 1000);

    if (!safeId || !safeProfileKey || !safeObservation) {
      return { success: false, reason: "required_value_missing", record: null };
    }

    const { data, error } = await supabase
      .from(INTERVENTION_TABLE)
      .update({ user_observation: safeObservation })
      .eq("id", safeId)
      .eq("profile_key", safeProfileKey)
      .eq("completed", true)
      .select("*")
    ...
    if (error) {
      return { success: false, reason: "database_error", error, record: null };
    }

    return { success: true, reason: "observation_recorded", record: data };
  } catch (error) {
    return { success: false, reason: "unexpected_error", error, record: null };
  }
}
