...
export function buildOrganisationSnapshot({
  ...
  const trendRows = [
    ...(baselineTrendRow ? [baselineTrendRow] : []),
    ...checkinTrendRows,
  ];
...
const movementSummary = {
  biggestImprovement: mostImproved
    ? {
        ...
        start: mostImproved.start,
        current: mostImproved.current,
  ...
  watchArea: watchArea
    ? {
        ...
        start: watchArea.start,
        current: watchArea.current,
 ...
 const initiative = recoveryRiskDetected
  ? {
      ...
      reason:
     "Burnout is currently the highest measured difficulty, while mood and focus are also elevated. Root recommends acting early to support recovery before these pressures become more established.",
