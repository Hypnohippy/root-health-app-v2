...
import { workforceDenominator } from "./organisationWorkforceContext.js";
import { buildOrganisationCommunication } from "./rootCommunicationEngine.js";
...
/*
 * Count every completed follow-up assessment, not only the
 * number of distinct people who have returned.
 */
const followUpCheckins = validJourneys.reduce(
  (total, journey) =>
    total +
    (Array.isArray(journey.checkins)
      ? journey.checkins.length
      : 0),
  0
);

/*
 * Compare each follow-up with that participant's immediately
 * preceding assessment.
 *
 * Higher scores mean greater difficulty:
 * negative movement = improvement
 * positive movement = worsening
 */
let improvingCheckins = 0;
let stableCheckins = 0;
let worseningCheckins = 0;
...
validJourneys.forEach((journey) => {
  ...
  for (
    let index = 1;
    index < participantAssessments.length;
    index += 1
  ) {
    const previousAssessment =
      participantAssessments[index - 1];

    const currentAssessment =
      participantAssessments[index];

    const changes = ORGANISATION_METRICS
      .map(([, key]) => {
        const previousValue = Number(
          previousAssessment?.[key]
        );
    ...
    if (averageMovement <= -0.25) {
      improvingCheckins += 1;
    } else if (averageMovement >= 0.25) {
      worseningCheckins += 1;
    } else {
      stableCheckins += 1;
    }
...
const totalAssessmentInteractions =
  baselineCompleted + followUpCheckins;
   ...
   */
  /*
 * ORGANISATIONAL PRIVACY GATES
 *
 * Baseline wellbeing evidence must not be released
 * until at least 5 distinct participants have a
 * valid baseline.
 *
 * Movement requires a separate threshold of at
 * least 5 participants with matched follow-up data.
 *
 * Root may still report participation counts below
 * these thresholds, but must not expose employee-
 * derived wellbeing scores, themes, severity,
 * hypotheses or targeted recommendations.
 */

const privacyMinimum = 5;

const baselineSuppressed =
  baselineCompleted < privacyMinimum;

const outcomeSuppressed =
  matchedParticipants < privacyMinimum;

  const participantOutcomesSuppressed = outcomeSuppressed ||
    [improvedParticipants, stableParticipants, worsenedParticipants].some(count => count > 0 && count < privacyMinimum);
  const checkinOutcomesSuppressed = outcomeSuppressed ||
    [improvingCheckins, stableCheckins, worseningCheckins].some(count => count > 0 && count < privacyMinimum);
  const participation = {
    // Legacy invited field remains a membership denominator for existing calculations.
    rootMembershipCount: organisation?.workforceContext?.rootMembershipCount ?? invited,
    invitationSentCount: organisation?.workforceContext?.invitationSentCount ?? null,
    linkedJoinedCount: organisation?.workforceContext?.joinedCount ?? null,
    invited,
    joined,
    baselineCompleted,
    matchedParticipants,
    ...
    followUpCheckins,
    totalAssessmentInteractions,
    improvingCheckins:
  checkinOutcomesSuppressed
    ? null
    : improvingCheckins,
...
worseningCheckins:
  checkinOutcomesSuppressed
    ...
    followUpRate,
    improvedParticipants: participantOutcomesSuppressed ? null : improvedParticipants,
    stableParticipants: participantOutcomesSuppressed ? null : stableParticipants,
    worsenedParticipants: participantOutcomesSuppressed ? null : worsenedParticipants,
    departmentsRepresented,
    participantOutcomesSuppressed,
    checkinOutcomesSuppressed,
  ...
  /*
 * Organisation Learning evidence.
 *
 * This contains organisation-level business measures supplied through
 * Organisation Learning Reviews. It is kept separate from anonymous
 * employee wellbeing evidence so Root can observe relationships without
 * implying that one caused the other.
 */
const orderedOrganisationReviews = chronologicalReviews(organisationReviews);

const latestOrganisationReview =
  orderedOrganisationReviews.length > 0
    ? orderedOrganisationReviews[
        orderedOrganisationReviews.length - 1
      ]
    : null;

const previousOrganisationReview =
  orderedOrganisationReviews.length > 1
    ? orderedOrganisationReviews[
        orderedOrganisationReviews.length - 2
      ]
    : null;

function buildBusinessMeasure(
  key,
  label,
  lowerIsBetter = true
) {
  const currentRaw =
    latestOrganisationReview?.[key];

  const previousRaw =
    previousOrganisationReview?.[key];
...
const organisationLearning = {
  businessEventNotes: latestOrganisationReview?.business_event_notes || "",
  initiativeNotes: latestOrganisationReview?.initiative_notes || "",
  reviewCount:
    orderedOrganisationReviews.length,

  latestReview:
    latestOrganisationReview,

  previousReview:
    previousOrganisationReview,

  measures:
    businessMeasures,

  businessEvents: Array.isArray(
    latestOrganisationReview?.business_events
  )
    ? latestOrganisationReview.business_events
    : [],

  initiatives: Array.isArray(
    latestOrganisationReview?.initiatives
  )
    ? latestOrganisationReview.initiatives
    : [],

  watchItems: Array.isArray(
    latestOrganisationReview?.watch_items
  )
    ? latestOrganisationReview.watch_items
    : [],
};
   ...
   *
   * These are factual reach measures, not an invented
   * engagement score.
   *
   * Reach = how much of the declared workforce has activated Root.
   * Depth = how much of the workforce has completed a baseline.
   * Continuity = how much of the workforce has returned with
   * matched follow-up evidence.
   *
   * workforce_size is the organisation's current declared
   * workforce denominator. Where the organisation was created
   * from a pricing band, this may currently represent the upper
   * bound of that band until an exact workforce figure is recorded.
   */

    /*
   * LEGACY COMPATIBILITY
   *
   * Keep this temporarily while older Root communication
   * code is migrated away from the old engagement score.
   *
   * It is no longer presented to HR as an engagement measure.
   */
  ...
  const workforceBasis = workforceDenominator(organisation);
  const workforceSize = workforceBasis.value;

  const workforceJoined = organisation?.workforceContext?.hasRecordedRoster
    ? organisation.workforceContext.joinedCount : activated;
  const workforceReachRate =
    workforceSize
      ? Math.min(
          100,
          Math.round((workforceJoined / workforceSize) * 1000) / 10
        )
      : null;

  const workforceDepthRate =
    workforceSize
      ? Math.min(
          100,
          Math.round((baselineCompleted / workforceSize) * 1000) / 10
        )
      : null;

  const workforceContinuityRate =
    workforceSize
      ? Math.min(
          100,
          Math.round((matchedParticipants / workforceSize) * 1000) / 10
        )
      : null;

  const workforceParticipation = {
    workforceContext: organisation?.workforceContext || null,
    denominatorSource: workforceBasis.source,
    membershipDenominator: invited,
    workforceSize,
    joined: workforceJoined,
    reachRate: workforceReachRate,
    baselineCompleted,
    depthRate: workforceDepthRate,
    returningParticipants: matchedParticipants,
    continuityRate: workforceContinuityRate,
  };
...
if (recoveryRiskDetected) {
  ...
  if (highRiskMetric?.label) {
    ...
    if (baselineCompleted < privacyMinimum) {
    nextReviewFocus.push(
      "Build sufficient baseline participation for anonymous organisational learning"
    );
  } else if (matchedParticipants < privacyMinimum) {
    nextReviewFocus.push(
      "Encourage continued participation so Root can begin measuring movement"
    );
  } else {
    nextReviewFocus.push(
      "Continue broadening workforce participation and repeated evidence"
    );
  }
...
/*
 * PRIVACY-SAFE ORGANISATION OUTPUT
 *
 * Everything returned to HR / leadership must pass
 * through this gate.
 *
 * Raw employee evidence may exist internally, but
 * organisation-facing intelligence is withheld until
 * the anonymous cohort threshold is reached.
 */

const releasedBaselineScore =
  baselineSuppressed
    ? null
    : baselineScore;

const releasedCurrentScore =
  baselineSuppressed
    ? null
    : currentScore;

const releasedMetricResults =
  baselineSuppressed
...
const releasedTrendRows =
  baselineSuppressed
    ? []
    : trendRows;

const releasedMostImproved =
  baselineSuppressed
    ? null
    : mostImproved;

const releasedWatchArea =
  baselineSuppressed
    ? null
    : watchArea;

const releasedHighRiskMetric =
  baselineSuppressed
    ? null
    : highRiskMetric;

const releasedMappedChallengeCounts =
  baselineSuppressed
    ? []
    : mappedChallengeCounts;

const releasedMostCommonTheme =
  baselineSuppressed
    ? "Protected until anonymity threshold is reached"
    : mostCommonTheme;

const releasedPrimaryConcern =
  baselineSuppressed
    ? "Awaiting anonymous workforce baseline"
    : primaryConcern;

const releasedRecommendedFocus =
  baselineSuppressed
    ? "Increase baseline participation"
    : recommendedFocus;

const releasedNextReviewFocus =
  baselineSuppressed
    ? [
        "Increase employee baseline participation",
        `Reach at least ${privacyMinimum} baseline participants`,
      ]
    : nextReviewFocus;

const releasedExecutiveStatus =
  baselineSuppressed
    ? {
        dot: "○",
        label: "Anonymous baseline developing",
        detail:
          `Root has ${baselineCompleted} completed employee baseline${
            baselineCompleted === 1 ? "" : "s"
          }. Wellbeing results remain protected until at least ${privacyMinimum} participants contribute.`,
      }
    : executiveStatus;

const releasedInitiative =
  baselineSuppressed
    ? {
        key: "baseline-participation",
        title: "Build the Workforce Baseline",
        introduction:
          "Root is currently building the anonymous workforce evidence base.",
        reason:
          `Wellbeing conclusions remain protected until at least ${privacyMinimum} employees have contributed a valid baseline.`,
        status: "Evidence developing",
        expectedOutcome:
          "A sufficiently anonymous workforce baseline that can support proportionate organisational decisions.",
        workshopTitle:
          "No targeted workshop recommended yet",
        workshopDescription:
          "Root will recommend targeted support once sufficient anonymous workforce evidence is available.",
        reportFocus:
          "Participation, anonymity and evidence quality.",
      }
    : initiative;

const releasedWorkforceNarrative =
  baselineSuppressed
    ? {
        insight:
          `Root has received ${baselineCompleted} completed employee baseline${
            baselineCompleted === 1 ? "" : "s"
          }. Individual-derived wellbeing findings remain protected until the anonymous reporting threshold of ${privacyMinimum} participants is reached.`,

        executiveSummary:
          `The workforce wellbeing picture is still protected. ${baselineCompleted} of the required ${privacyMinimum} anonymous baseline participants are currently contributing.`,

        watchAreas: [
          "Continue building anonymous baseline participation.",
          "No wellbeing pressure point will be released before the anonymity threshold is reached.",
        ],

        learning: [
          "Participation and Wellbeing Awareness",
          "Understanding Root's Anonymous Reporting Model",
        ],
...
const releasedOrganisationMemory =
  baselineSuppressed
    ? [
        `Root currently has ${baselineCompleted} valid employee baseline${
          baselineCompleted === 1 ? "" : "s"
        }. Wellbeing findings remain protected until ${privacyMinimum} participants contribute.`,
        "Root will begin building organisation-level wellbeing memory only after the anonymous cohort threshold is reached.",
      ]
    : organisationMemory;

const releasedExecutiveQuestions =
  baselineSuppressed
    ? [
        "How can the organisation encourage sufficient participation while reinforcing employee privacy and trust?",
        `What would help Root reach the anonymous baseline threshold of ${privacyMinimum} participants?`,
      ]
    : executiveQuestions;

const releasedRootHypothesis =
  baselineSuppressed
    ? "Root is deliberately withholding an organisational wellbeing hypothesis because the anonymous baseline threshold has not yet been reached."
    : rootHypothesis;

const releasedRecommendedInsight =
  baselineSuppressed
    ? {
        title:
          "Building a trustworthy anonymous workforce baseline",
        slug:
          "wellbeing-baseline",
        reason:
          "The priority is currently participation, privacy and evidence quality rather than interpretation of wellbeing scores.",
      }
    : recommendedInsight;

const releasedConfidenceReasons =
  baselineSuppressed
    ? [
        `${baselineCompleted} distinct employee baseline${
          baselineCompleted === 1 ? "" : "s"
        } completed.`,
        `${privacyMinimum} participants are required before organisation-level wellbeing results are released.`,
        `${activated} employee${
          activated === 1 ? "" : "s"
        } activated.`,
      ]
...
const releasedBoardCase =
  baselineSuppressed
    ? {
        executiveSummary:
          "Root is building the anonymous workforce evidence base. Wellbeing findings remain protected until the minimum reporting cohort is reached.",

        organisationalRisk:
          "No specific workforce wellbeing risk is being reported at this stage because the anonymous cohort threshold has not been reached.",

        supportingEvidence: [
          `${baselineCompleted} completed employee baseline${
            baselineCompleted === 1 ? "" : "s"
          }`,
          `${activated} activated employee${
            activated === 1 ? "" : "s"
          }`,
          `Anonymous reporting threshold: ${privacyMinimum} participants`,
        ],

        recommendation:
          "Continue building anonymous employee participation before making a targeted wellbeing decision.",

        expectedBenefit:
          "A trustworthy workforce baseline that protects individual privacy and supports proportionate future action.",

        costOfInaction:
          "Insufficient participation would prevent Root from producing a reliable anonymous organisation-level wellbeing picture.",

        confidence:
          "Evidence Protected",

        approvalRequest:
          "Support employee participation and reinforce Root's privacy protections while the anonymous baseline develops.",

        successCriteria: [
          `Reach at least ${privacyMinimum} valid baseline participants.`,
          "Maintain employee trust and privacy.",
          "Establish the first anonymous workforce wellbeing picture.",
        ],

        reviewQuestions: [
          "Has baseline participation reached the anonymous reporting threshold?",
          "Are employees clear that their individual wellbeing information remains private?",
        ],
      }
    : boardCase;
const releasedAnalysisStage =
  baselineSuppressed
    ? {
        level: 0,
        title: "Awaiting Workforce Baseline",
        description: `Root is waiting for at least ${privacyMinimum} completed employee baseline assessments before releasing an anonymous organisation-level wellbeing picture.`,
      }
    : analysisStage;

const releasedExecutiveNarrative =
  baselineSuppressed
    ? {
        overview:
          "Root does not yet have sufficient anonymous workforce evidence to establish an organisation-level wellbeing baseline.",

        numbersSuggest:
          "No employee wellbeing scores are being released until the anonymous reporting threshold is reached.",

        stressCommentary:
          "No anonymous organisation-level stress result is available yet.",

        burnoutCommentary:
          "No anonymous organisation-level burnout result is available yet.",

        recoverySleepCommentary:
          "No anonymous organisation-level sleep or recovery results are available yet.",

        additionalCommentary:
          "No anonymous organisation-level mood or focus results are available yet.",

        detected:
          "No workforce wellbeing pattern has been released yet. Root is currently building the anonymous evidence base.",

        meaning:
          "There is not yet enough anonymous workforce evidence to support an organisational wellbeing interpretation.",

        watchingNext:
          "Root will first watch participation and baseline completion. Once the anonymous reporting threshold is reached, it can begin identifying the areas that genuinely deserve leadership attention.",
        ...
        forecast:
          "As employees complete baseline assessments, Root will establish the organisation's first anonymous wellbeing picture. Later matched check-ins will then allow Root to identify genuine movement.",

        recommendation:
          "Root recommends continuing to build employee participation until the anonymous workforce baseline can be safely released.",

        recommendationReason:
          "Root is deliberately withholding a targeted wellbeing recommendation because the anonymous reporting threshold has not yet been reached.",

        businessImpact:
          "Root does not yet have sufficient anonymous workforce wellbeing evidence to estimate the organisational significance of any specific wellbeing issue.",
        ...
        boardApproval:
          "No targeted wellbeing initiative requires board approval at this stage. Leadership should support participation and allow Root to establish a reliable anonymous baseline first.",

        successMeasures: [
          "Increase baseline participation.",
          `Reach the anonymous reporting threshold of ${privacyMinimum} participants.`,
          "Establish a reliable anonymous starting wellbeing position.",
          "Create sufficient evidence for the next Executive Review.",
        ],

        closingSummary:
          "This report does not yet establish an anonymous workforce wellbeing baseline. Root currently has organisation context and participation information, but wellbeing conclusions remain protected until the reporting threshold is reached.",
      }
    : executiveNarrative;
  ...
  buildOrganisationCommunication(
    {
      ...
      baselineScore:
        releasedBaselineScore,

      currentScore:
        releasedCurrentScore,

      metricResults:
        releasedMetricResults,

      trendRows:
        releasedTrendRows,

      mostImproved:
        releasedMostImproved,

      watchArea:
        releasedWatchArea,

      highRiskMetric:
        releasedHighRiskMetric,

      movementSummary,

      mostCommonTheme:
        releasedMostCommonTheme,

      mappedChallengeCounts:
        releasedMappedChallengeCounts,

      primaryConcern:
        releasedPrimaryConcern,

      recommendedFocus:
        releasedRecommendedFocus,

      confidenceScore,
      confidenceLabel,

      executiveStatus:
        releasedExecutiveStatus,

      initiative:
        releasedInitiative,

      nextReviewFocus:
        releasedNextReviewFocus,

      analysisStage:
        releasedAnalysisStage,
    },
    {
      audience: "executive",
    }
...
workforceNarrative:
  releasedWorkforceNarrative,

executiveEvidence:
  baselineSuppressed
    ? `${baselineCompleted} of ${privacyMinimum} anonymous baseline participants`
    : executiveEvidence,

organisationMemory:
  releasedOrganisationMemory,

executiveQuestions:
  releasedExecutiveQuestions,

rootHypothesis:
  releasedRootHypothesis,

recommendedInsight:
  releasedRecommendedInsight,

confidenceReasons:
  releasedConfidenceReasons,
...
boardDecision: {
  recommendation:
    releasedBoardCase.recommendation,

  approvalRequest:
    releasedBoardCase.approvalRequest,

  expectedBenefit:
    releasedBoardCase.expectedBenefit,

  successCriteria:
    releasedBoardCase.successCriteria,

  reviewQuestions:
    releasedBoardCase.reviewQuestions,
},
