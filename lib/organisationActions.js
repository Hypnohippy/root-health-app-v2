...
export async function listOrganisationActions({ supabase, organisationId, offset = 0, limit = 50, status }) {
  ...
  if (status && !enums.status.includes(status)) throw new Error('Invalid status');
  let query = supabase.from('organisation_actions').select(ACTION_FIELDS, { count: 'exact' }).eq('organisation_id', organisationId);
