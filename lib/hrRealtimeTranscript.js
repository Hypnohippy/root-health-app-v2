...
export function createHRRealtimeTranscript(sessionId) {
  ...
  function finish(output, completed = false) {
    ...
    // Terminal items need not repeat the transcript already received in deltas.
    // Preserve that text and release its controls only once completion is known.
    if (text || (completed && entry.content)) entry.final = true;
  }
  return {
    consume(event) {
      ...
      if (["conversation.item.added", "conversation.item.created", "conversation.item.done", "response.output_item.added", "response.output_item.done"].includes(type)) {
        if (event.item?.role === "user" || event.item?.role === "assistant") {
          ...
          finish(event.item, type.endsWith(".done") && event.item.status !== "incomplete");
        }
      }
      ...
      if (type === "response.done") for (const output of event.response?.output || []) finish(output, event.response.status === "completed");
      return this.entries();
    },
