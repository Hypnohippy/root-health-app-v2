...
export function formatHealthContextForPrompt(value) {
  const state = classifyHealthContextValue(value);
  if (!state.responseProvided) return "unknown — no explicit response recorded";
  if (state.knowledge === "known_absence") return "known absence";
  if (state.knowledge === "unknown") {
    return "unknown — the user's response is uncertain, ambiguous or withheld";
  }
  return state.value;
}
