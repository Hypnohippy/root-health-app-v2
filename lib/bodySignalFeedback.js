...
function practicalGuidance(row, profile) {
  ...
  // The persisted system is authoritative. Symptom words are only a fallback
  // for legacy rows without a canonical system; words such as "pressure" are
  // otherwise too ambiguous to choose an unrelated Body pathway.
  if (/nervous|sensory/.test(system)) {
    guidance.push("A brief reduction in noise, screens, bright light or multitasking may give you a useful comparison point.");
  } else if (/circulatory|respiratory/.test(system)) {
  ...
  } else if (system.includes("digestive")) {
  ...
  } else if (system.includes("musculoskeletal")) {
  ...
  } else if (!system && /overwhelm|racing thoughts|wired|sensory|eye strain|light sensitivity/.test(text)) {
    guidance.push("A brief reduction in noise, screens, bright light or multitasking may give you a useful comparison point.");
  } else if (!system && /tight chest|pressure|breathless|wheeze|racing heart|flutter/.test(text)) {
    guidance.push("Pause exertion and reduce demand while you notice whether the signal settles; do not push through chest or breathing symptoms.");
  } else if (!system && /bloating|reflux|nausea|cramp|appetite|food sensitivity/.test(text)) {
    guidance.push("Keep your usual clinical and dietary requirements in place, and notice timing around meals, hydration and stress rather than assuming one trigger from a single entry.");
  } else {
