...
export function createBodySignalDraft(system, locationDetail = "") {
  return {
    system,
    locationDetail,
    symptoms: [],
...
export function buildBodyCoachHandoff(savedRecord, existingJourney = null) {
  if (!savedRecord?.id) return null;
  return {
    ...(existingJourney && typeof existingJourney === "object" ? existingJourney : {}),
    currentStage: "coach",
    completedBody: true,
    bodySignalId: savedRecord.id,
    bodyObservation: {
      id: savedRecord.id,
      areas: cleanBodyValues(savedRecord.areas || []),
      system: savedRecord.system || "",
      location_detail: savedRecord.location_detail || null,
      symptoms: cleanBodyValues(savedRecord.symptoms?.length ? savedRecord.symptoms : [savedRecord.signal]),
      timing_contexts: cleanBodyValues(savedRecord.timing_contexts?.length ? savedRecord.timing_contexts : [savedRecord.context]),
      duration_patterns: cleanBodyValues(savedRecord.duration_patterns || []),
      intensity: Number.isFinite(Number(savedRecord.intensity)) ? Number(savedRecord.intensity) : null,
      modifiers: cleanBodyValues(savedRecord.modifiers?.length ? savedRecord.modifiers : [savedRecord.what_helped]),
      notes: savedRecord.notes || null,
      created_at: savedRecord.created_at || null,
    },
    bodyAreas: cleanBodyValues(savedRecord.areas || []),
    selectedSignal: savedRecord.symptoms?.[0] || savedRecord.signal || "",
    intensity: Number.isFinite(Number(savedRecord.intensity)) ? Number(savedRecord.intensity) : null,
  };
}

export function summariseStructuredBodyObservation(record) {
  if (!record) return "No current Body observation was supplied.";
  const values = (value) => cleanBodyValues(Array.isArray(value) ? value : [value]).join(", ") || "not recorded";
  return [
    `Body record ID: ${record.id || "not recorded"}`,
    `Area/system: ${values(record.areas)} / ${record.system || "not recorded"}`,
    `Specific location: ${record.location_detail || "not recorded"}`,
    `Symptoms: ${values(record.symptoms?.length ? record.symptoms : record.signal)}`,
    `Timing/context: ${values(record.timing_contexts?.length ? record.timing_contexts : record.context)}`,
    `Duration/pattern: ${values(record.duration_patterns)}`,
    `Intensity: ${record.intensity ?? "not recorded"}/10`,
    `Modifiers/helpful factors: ${values(record.modifiers?.length ? record.modifiers : record.what_helped)}`,
    `User notes: ${record.notes || "not recorded"}`,
  ].join("\n");
}
