...
export function createPersonalInterventionLifecycle({
  ...
  async function completeActive({ afterScore = null, userObservation = null } = {}) {
    ...
    if (result?.success) {
      completedAttempt = { ...attempt, result };
      activeAttempt = null;
    }
  ...
  async function observeCompleted(userObservation) {
    if (!completedAttempt || typeof observe !== "function") {
      return { success: false, reason: "completed_attempt_missing", record: null };
    }

    return observe({
      interventionId: completedAttempt.result.record.id,
      profileKey: completedAttempt.payload.profileKey,
      userObservation,
    });
  }
