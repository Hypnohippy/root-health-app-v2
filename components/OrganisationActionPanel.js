...
export default function OrganisationActionPanel({ access }) {
  ...
  }, [access?.accessToken, access?.organisationId, retry]);
  ...
  return <section className="root-actions" aria-labelledby="root-actions-title">
    ...
    <style jsx>{`
      ...
      strong {flex:1;min-width:180px} .pill,dt {text-transform:capitalize} dl div {margin:12px 0} dt {font-weight:600} dd {margin:4px 0;white-space:pre-wrap;overflow-wrap:anywhere} a {color:#6f675b} .action-row {font-size:14px}
    `}</style>
  </section>;
}
